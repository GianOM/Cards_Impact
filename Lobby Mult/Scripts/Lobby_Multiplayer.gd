...
#Sinal a ser enviado mostrando que o Lobby terminou de ser construido
signal Terminou_de_Construir_o_Lobby


#Sinal usada para avisar que a lista de players mudou totalmente para atualizar parametros
...
func Create_Multiplayer_Game():
	...
	List_of_Players[1].name = "O ADM"#Atualiza o nome do host para ser o seu ID, neste caso 1
	
	#player_connected.emit(1, Player_Basic_Info)
	
	Terminou_de_Construir_o_Lobby.emit()
...
func _register_player(new_player_info):
	...
	List_of_Players[multiplayer.get_remote_sender_id()] = new_player_info
	
	print(List_of_Players)
	
	#Usar a seguinte forma para emitir um sinal que passará parametros
	#Para determinada funcao
	Player_2_Registrado_no_Host.emit()
	#emit_signal("Player_2_Registrado_no_Host", multiplayer.get_remote_sender_id())
...
func _on_steam_lobby_created(connected, id):
	if connected:
		...
		List_of_Players[1] = {"name": Steam_UserName,"is_Player_Ready": false}
