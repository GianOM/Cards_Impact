...
func _generate_grid():
	for x in range(TILE_GRID_SIZE.x):
		...
		for y in range(TILE_GRID_SIZE.y):
			...
			#
			#add_child.call_deferred(Hex_Tile_Instance)
			...
			if Hex_Tile_Instance.Check_Overlap_Collision(Vector3(global_position.x + Tile_Placement_Coordinates.y,0, global_position.z + Tile_Placement_Coordinates.x)) == false:
