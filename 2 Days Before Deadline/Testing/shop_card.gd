...
const CARD_MENU_UI = preload("res://Testing/card_menu_ui.tscn")
...
@onready var coin_cost := randi_range(30, 150)

var current_card_ui: CardMenuUI

#func _ready() -> void:
	#update(preload("res://Testing/test_data/test_run_stats.tres"))

func update(run_stats: RunStats) -> void:
	if not card_container or not price or not buy_button:
		return
	
	price_label.text = str(coin_cost)
	
	if run_stats.coins >= coin_cost:
...
func _on_buy_button_pressed() -> void:
	Events.shop_card_bought.emit(card, coin_cost)
